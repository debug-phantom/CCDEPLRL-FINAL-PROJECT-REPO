...
st.markdown("""
Choose between **CNN** (custom, 224×224) and **MobileNetV2** (transfer learning).  
- **Both models max 20 epochs** – CNN is optimized for speed (fewer layers).  
...
MobileNetV2 gives higher confidence (80‑95%) but is still efficient.
""")
...
# -------------------------------------------------------------
# 4. FASTER CNN TRAINING FUNCTION (224x224, fewer layers)
# -------------------------------------------------------------
def train_cnn(dataset_path, target_accuracy=None):
    tf.random.set_seed(42)
    np.random.seed(42)
    
    # Fixed 224x224 but simplified architecture
    img_size = (224, 224)
    ...
    model = models.Sequential([
        ...
        layers.BatchNormalization(),
        layers.MaxPooling2D(),
        # No extra Conv2D(512) – removed for speed
        layers.Dropout(0.3),
        layers.Flatten(),
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint("cnn_model.keras", save_best_only=True, monitor='val_accuracy', mode='max')
    ]
...
# -------------------------------------------------------------
# 5. MOBILENETV2 TRAINING FUNCTION (224x224, max 20 epochs)
# -------------------------------------------------------------
...
if st.sidebar.button("🚀 Train Model"):
    with st.spinner(f"Training {model_choice}... This may take a few minutes."):
        if model_choice == "CNN":
            ...
            st.success("✅ CNN model (224x224, faster architecture) trained and saved as 'cnn_model.keras'")
        else:
